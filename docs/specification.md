...
| `openpapir import --archive <root> <file>... [--case <case-id> --description <d> [--date <yyyy-mm-dd>]] [--json]` | Stores each file's original bytes in the content-addressed artefact store and records one import event per input. With `--case` and `--description` it also records one submission naming every imported artefact, under the same writer lock. |
...
| `openpapir submission add --archive <root> --case <case-id> --description <d> [--date <yyyy-mm-dd>] [--artefact <digest>[:<role>]]... [--file <path>[:<role>]]... [--json]` | Records one submission the user states they sent, against a case. `--file` imports the file and references its digest in the same record, under the same writer lock. |
