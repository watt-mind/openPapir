...
naming the `shell` argument; the value typed is never echoed back. There is no
`--archive` and no `--json`: a completion script is not a result to report in
two forms.
...
for one. A reader pages through the whole stream with
`openpapir manpage | man -l -`, and an installer that wants one file per
