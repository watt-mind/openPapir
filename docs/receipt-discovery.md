...
repository; no third-party schema or document content is redistributed here.
...
- The byte-level container of a *Feladási igazolás* (PDF, XML, `.es3`, KRX, or
  ...
  present.
- Any field that identifies the originating submission, and whether such a
  field is stable, unique, or user-visible.
- Whether the same artefact shape is produced for every case type, every
  authority, and every mailbox kind (personal, Cégkapu, Hivatali kapu).
...
The evidence base is thin: one operator help page carried nearly all the
descriptive detail, the normative source was retrieved incompletely, and two
sources were unreachable. Conclusions about receipt structure are therefore
absent by design rather than summarised. Do not treat any proposal in the local
case model section as a decision until it is restated in a bounded issue with
acceptance criteria.
