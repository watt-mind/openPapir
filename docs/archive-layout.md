...
rejection, but the allow list is an explicit list and did not include it when
this review ran. It does now: the decision the first bullet of What would
change the answer below asked for was taken separately on 2026-09-10, and
`deny.toml` carries `BSD-3-Clause` with its reasons in a comment beside the
list. The measurement above is left as it was taken.
...
- A deliberate decision to add `BSD-3-Clause` to the `deny.toml` allow list,
  ...
  That is the whole gap. **This has since happened.** The operator took that
  decision on 2026-09-10, on the grounds recorded in the comment beside the
  allow list in `deny.toml`: the licence is permissive on the same footing as
  the ones already allowed, and its only extra obligations are attribution and
  the no-endorsement clause. The attribution obligation is met by
  `THIRD-PARTY-NOTICES.md`, generated by `scripts/third-party-notices.py` from
  `cargo metadata` and carried by every release archive. No crate was added by
  that change, and this review still admits none: what it removes is the
  licence objection, so a future review of a container crate is a crate
  decision on cost, `unsafe`, advisories, and maintenance, decided by
  re-running the commands above against the versions of its own date.
...
- **Encrypted backup at rest** (new): **not implemented**, decided above.
  ...
  `deny.toml` did not allow. The allow list has since been decided on its own
  terms, also on 2026-09-10, and now carries `BSD-3-Clause`, so the licence
  objection is gone. The follow-up stays closed all the same: admitting a
  container crate needs a review of that crate, taken on its own date against
  the versions published then, and no such review has been run since the
  licence decision. Nothing here proposes one.
