...
- `openpapir search --archive <root> <text>
  [--kind case|submission|receipt|association]... [--json]`, operation
  `search`, reports where a word is in the user's own record text. It
  reads case titles, notes, and tags, submission descriptions, receipt labels,
  an association's own statement, and the record identifiers, and it reads no
  stored object, no original filename, and nothing derived. The match folds
  both sides to lower case and looks for a substring, exactly as
  `case list --query` does, over one linear scan per kind and no index. `data`
  holds `hits`, `count`, and `kinds`; each hit carries `kind`, `id`, `field`,
  and `case_id` when the record belongs to a case, ordered by kind, then by
  identifier, then by field name. A hit names the field and never the matched
  text, and the query is never echoed back. `--kind` may be repeated and
  narrows what is read; an unknown kind is `usage.arguments`. The query is
  bounded at 4096 bytes by `input.cap.field_length` naming `query`, and a
  query that matches nothing is a success with `count` `0`. The command takes
  no lock and changes nothing.
- Every release archive now carries the shell completion scripts under
  `completions/` and the man page at `man/man1/openpapir.1` beside the binary,
  each generated by the built binary of that target while the archive is
  staged, and the archive's `.sha256` file covers them. The staging and its
  layout check moved into `scripts/package-release.sh`, which the release
...
- The dependency licence allow list in `deny.toml` carries `BSD-3-Clause`,
  decided on 2026-09-10 on its own terms and with those terms recorded in a
  comment beside the list: the licence is permissive on the same footing as
  the ones already allowed, and its only extra obligations are attribution and
  the clause forbidding use of the copyright holder's name to endorse a
  derived work. No dependency was added, removed, or changed by this, and no
  other licence, ban, source, or advisory setting moved. The attribution
  obligation is now met in the artefacts: `THIRD-PARTY-NOTICES.md` lists every
  dependency a shipped binary links, with its version and the licence
  expression it declares, and every release archive carries it beside
  `LICENSE`, so `scripts/package-release.sh --describe` and
  `--describe --paths` name it. The file is generated by
  `scripts/third-party-notices.py` from `cargo metadata`, needing cargo and
